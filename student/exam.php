		    ...
		    <form role="form" method="POST" id="fetchStudentDataForm">
				<div class="card-body">
				    <!--<div class="card-header">
								<h3 class="card-title">FILLING UP OF EXAMINATION FORM HAS BEEN CLOSED.</h3>
							</div>-->
		            <div class="row">	
                    <?php 
                       $sql_sem = "SELECT * FROM `tbl_semester`
                             WHERE `course_id` = $course_id && `fee_academic_year` = '".$row["admission_session"] ."' && `fee_status` = 'Active'";
                      //var_dump("SELECT * FROM `tbl_semester`
                            //WHERE `course_id` = $course_id && `fee_academic_year` = '".$row["admission_session"] ."' && `fee_status` = 'Active'");
                           $sem_result = $con->query($sql_sem);
					       $sem_row = $sem_result->fetch_assoc();
                       
                   // echo($sem_row["exam_fee_last_date"]);  
                      $today = date("Y-m-d");
                     // echo $today;
	...
	$(function(){
		...
		function showdesg() {
			...
			$.ajax({
				url: '../ajaxdata3.php',
				type: 'POST',
				data: {depart: dept,academic_year: academic_year},
